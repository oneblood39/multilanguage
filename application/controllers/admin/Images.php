...
class Images extends Admin_Controller
{
	...
	public function index($content_id = NULL) //upload edilecek fotoğrafların anasayfası
	{
        if(!isset($content_id))
        {
    ...
    public function edit_title($image_id) //fotoğraf başlıklarını değiştirme
    {
    ...
    public function featured($content_id)   //karşıya yükleme özelliklerinin düzenlendiği alan
    {
    ...
    public function delete_featured($content_id)  /// default görsel kaldırma
    {
        $content = $this->content_model->get($content_id);
        if($content===FALSE)
        {
    ...
    public function delete($image_id)   //dosya silme
    {
        $image = $this->image_model->get($image_id);
        $file = $image->file;
