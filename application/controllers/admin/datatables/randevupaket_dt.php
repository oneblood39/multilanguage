...
class Randevupaket_dt extends Admin_Controller
{
  ...
  public function getall(){
   ...
   $data = array();
    ...
    $query = $this->db->query("SELECT COUNT(randevuID) as total  FROM vwrandevu where paketID!=''");
...
".$filtre." ORDER BY danisanID desc LIMIT ".$start.",".$length;
    ...
    // if($search){
       if($cat->danisanID>0){  
        ...
        $data .= '["'.$Ad.'","'.$Soyad.'","'.$Dad.'","'.$Dsoyad.'","'.$terapitip.'","'.$tarih.'","'.$paketadi.'","'.$yazi.'","'.$durum.'"," <a href=\"'.site_url('admin/terapi/cagri/cagridetay/').$cat->danisanID.'\"></a>"],';
  //print_r($data);
